...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        # TODO
        out_loss = torch.log(torch.sigmoid(torch.sum(input_vectors * output_vectors, dim=1)))

        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        noise_loss = torch.sum(torch.log(torch.sigmoid(-torch.sum(input_vectors.unsqueeze(1) * noise_vectors, dim=2))), dim=1)
